...
import           Xenon.Analyze.CounterExample (generateCounterExampleGraphs)
import           Xenon.Language.Annotation
import           Xenon.Language.IR (prettyShow, Module(..))
...
import           Xenon.Types
import           Xenon.Analyze.ModuleSummary (SummaryMap)
import           Xenon.Utils
...
-- -----------------------------------------------------------------------------
generateIR :: XenonArgs -> IO (XenonArgs, AnnotationFile)
-- -----------------------------------------------------------------------------
generateIR XenonArgs{..} = do
  ...
  let result = XenonArgs { fileName   = irFile
...
-- -----------------------------------------------------------------------------
checkIR :: (XenonArgs, AnnotationFile) -> IO Bool
-- -----------------------------------------------------------------------------
checkIR (ia@XenonArgs{..}, af)
  | printIR = do
...
computeFInfo :: XenonArgs -> AnnotationFile -> IO PipelineOutput
computeFInfo ia@XenonArgs{..} af = do
  irFileContents <- readIRFile ia fileName
...
handleMonads :: XenonArgs
             -> ErrorC XenonException (WriterC Output IO) a
             -> IO (Either XenonException a)
handleMonads ia act = handleOutput ia $ runError act

handleTrace :: XenonArgs -> TraceC m a -> m a
handleTrace XenonArgs{..} =
  if verbosity /= Loud || benchmarkMode
  then runTraceIgnore
  else runTracePrint

handleOutput :: MonadIO m => XenonArgs -> WriterC Output m a -> m a
handleOutput XenonArgs{..} act =
...
readIRFile :: XenonArgs -> String -> IO String
...
errorHandle :: XenonException -> IO a
