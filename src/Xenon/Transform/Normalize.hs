...
module Xenon.Transform.Normalize
  ( normalize
  , assignThreadIds
  , NormalizeOutput
  , NormalizeIR
...
makeLenses ''St
makeLenses ''StmtSt
type G sig m = (Has (Error XenonException) sig m, Has CET.Trace sig m)
...
normalizeModule :: (FD sig m, Has (Error XenonException) sig m)
